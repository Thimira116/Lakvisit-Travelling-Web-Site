...
$images = $conn->query("SELECT * FROM services_images WHERE ad_id = $id");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['feedback'])) {
    ...
    // Insert feedback
    $conn->query("INSERT INTO services_feedback (ad_id, username, comment) VALUES ($id, '$username', '$comment')");
...
// Get feedbacks as a PHP array to avoid pointer issues
$feedbacks_result = $conn->query("SELECT * FROM services_feedback WHERE ad_id = $id ORDER BY created_at DESC");
$feedbacks = $feedbacks_result->fetch_all(MYSQLI_ASSOC);


// Get related ads in the same district (excluding current ad)
$related_stmt = $conn->prepare("SELECT * FROM services_ads WHERE district = ? AND id != ? LIMIT 6");
...
<head>
    ...
    @media (min-width: 1200px) {
      #adCarousel .carousel-item img {
        height: 500px; /* Bigger for large desktops */
      }
    }
...
<div class="container my-4">
    ...
    <h2 class="mb-3"><?= html_entity_decode($post['title']) ?></h2>
...
<?php if ($images->num_rows > 0): ?>
  <div id="adCarousel" class="carousel slide mb-3" data-bs-ride="carousel">
    ...
    <div class="carousel-inner">
      <?php 
      $active = "active";
      // Reset pointer so loop works twice (indicators + images)
      $images->data_seek(0); 
      ...
      <?php 
      $active = ""; // only the first one is active
      endwhile; ?>
    </div>
...
<?php endif; ?>
    ...
    <p><strong>Location:</strong> <?= htmlspecialchars($post['location']) ?>, <?= htmlspecialchars($post['district']) ?>, <?= htmlspecialchars($post['province']) ?></p>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js">
  // Reload the page if loaded from the back/forward cache (bfcache)
